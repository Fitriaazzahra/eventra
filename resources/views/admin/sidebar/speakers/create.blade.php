...
@section('title', app()->getLocale() === 'id' ? 'Tambah Pembicara' : 'Add Speaker')
@section('breadcrumb', app()->getLocale() === 'id' ? 'Pembicara / Tambah' : 'Speakers / Add')

@section('content')
    <div class="eventra-heading-row">
        <div>
            <h1>{{ app()->getLocale() === 'id' ? 'Tambah Pembicara' : 'Add Speaker' }}</h1>
            <p>{{ app()->getLocale() === 'id' ? 'Tambahkan profil pembicara baru untuk event.' : 'Add a new speaker profile for your events.' }}</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.speakers.store', ['locale' => app()->getLocale()]) }}" class="eventra-form-shell">
        @csrf
                ...
                <div class="eventra-form-card">
                    ...
                    <div class="eventra-form-grid">
                        <div class="eventra-form-field">
                            ...
                            <input type="text" name="job_title" value="{{ old('job_title') }}" class="eventra-form-input">
                        </div>
                        <div class="eventra-form-field">
                            <label>{{ app()->getLocale() === 'id' ? 'Perusahaan' : 'Company' }}</label>
                            <input type="text" name="company" value="{{ old('company') }}" class="eventra-form-input">
                        </div>
                    </div>

                    <div class="eventra-form-field">
                        <label>{{ app()->getLocale() === 'id' ? 'Biografi' : 'Biography' }}</label>
                        <textarea name="biography" class="eventra-form-textarea" rows="5">{{ old('biography') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="eventra-form-side-column">
                <div class="eventra-form-card">
                    ...
                    <div class="eventra-form-field">
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" class="eventra-form-input">
                    </div>
                    <div class="eventra-form-field">
                        <label>LinkedIn</label>
                        <input type="url" name="linkedin" value="{{ old('linkedin') }}" class="eventra-form-input">
                    </div>
                    <div class="eventra-form-field">
                        <label>{{ app()->getLocale() === 'id' ? 'Website' : 'Website' }}</label>
                        <input type="url" name="website" value="{{ old('website') }}" class="eventra-form-input">
                    </div>
                </div>
